        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                <x-dropdown align="right" width="80">
                    <x-slot name="trigger">
                        <button class="relative inline-flex items-center p-2 mr-4 text-gray-500 hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            ...
                            @if(Auth::user()->unreadNotifications->count() > 0)
                                <span class="absolute top-1 right-1 flex h-3 w-3">
                                    ...
                                    <span class="relative inline-flex rounded-full h-3 w-3 bg-red-500 text-[9px] text-white font-bold items-center justify-center">{{ Auth::user()->unreadNotifications->count() }}</span>
                                </span>
                            @endif
                        </button>
                    </x-slot>
                    ...
                    <x-slot name="content">
                        ...
                        <div class="max-h-60 overflow-y-auto">
                            @forelse(Auth::user()->notifications()->limit(10)->get() as $notification)
                                <a href="{{ $notification->data['url'] ?? '#' }}" class="block px-4 py-3 border-b border-gray-100 hover:bg-gray-50 transition {{ $notification->read_at ? 'opacity-75' : 'bg-teal-50/30' }}">
                                    <div class="flex items-start">
                                        ...
                                        <div class="ml-3">
                                            <p class="text-xs font-bold {{ $notification->read_at ? 'text-gray-700' : 'text-gray-900' }}">{{ $notification->data['title'] ?? 'Pemberitahuan' }}</p>
