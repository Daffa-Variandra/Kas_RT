<x-sidebar-layout>
    ...
    <div x-data="{ showModal: {{ $errors->any() ? 'true' : 'false' }} }">
        ...
        <div class="mb-6 flex justify-between items-center">
            <h3 class="text-lg font-bold text-gray-700">Daftar Tagihan Smart RW</h3>
            <button @click="showModal = true" class="bg-emerald-600 hover:bg-emerald-900 text-white px-4 py-2 rounded-xl text-sm font-bold transition flex items-center shadow-lg shadow-teal-100">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-sm font-bold text-teal-700">Rp {{ number_format($item->nominal, 0, ',', '.') }}</span>
                            </td>
                            <td class="px-6 py-4 text-center" x-data="{ editOpen: false, deleteOpen: false }">
                                <div class="flex justify-center space-x-3">
                                    <button @click="editOpen = true" class="text-teal-600 hover:text-teal-900 text-xs font-bold uppercase tracking-tighter transition-colors">Edit</button>
                                        ...
                                        <form action="{{ route('admin.contributions.update', $item->id) }}" method="POST" class="p-6">
                                            ...
                                            <div class="space-y-4">
                                                <div>
                                                    ...
                                                    <input type="text" name="nama_iuran" value="{{ $item->nama_iuran }}" class="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-sm" required>
                                                </div>
                                                <div>
                                                    <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Nominal (Rp)</label>
                                                    <input type="number" name="nominal" value="{{ (int)$item->nominal }}" class="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-sm" required>
                                                </div>
                                                <div>
                                                    <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Deskripsi / Keterangan</label>
                                                    <textarea name="deskripsi" rows="2" class="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-sm">{{ $item->deskripsi }}</textarea>
                                                </div>
                                            </div>
                                            <div class="mt-6 flex gap-3">
                                                ...
                                                <button type="submit" class="flex-[2] px-4 py-2.5 text-sm font-bold text-white bg-emerald-600 rounded-lg hover:bg-emerald-900 shadow-md">Update Data</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
            ...
            <div @click.away="showModal = false" class="relative bg-white rounded-2xl shadow-2xl w-full sm:w-96 z-10 overflow-hidden">
                ...
                <form action="{{ route('admin.contributions.store') }}" method="POST" class="p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Nama Iuran</label>
                            <input type="text" name="nama_iuran" class="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-sm" placeholder="Contoh: Iuran Bulanan Warga" required>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Nominal (Rupiah)</label>
                            <input type="number" name="nominal" class="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-sm" placeholder="Contoh: 50000" min="0" required>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Deskripsi (Opsional)</label>
                            <textarea name="deskripsi" rows="2" class="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-sm" placeholder="Keterangan tagihan..."></textarea>
                        </div>
                    </div>
                    <div class="mt-6 flex gap-3">
                        ...
                        <button type="submit" class="flex-[2] px-4 py-2.5 text-sm font-bold text-white bg-emerald-600 rounded-lg hover:bg-emerald-900 shadow-md">Simpan Tagihan</button>
                    </div>
                </form>
            </div>
        </div>
