    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8" x-data="{ proofModal: false, activeImg: '' }">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden sticky top-6">
                <div class="px-6 py-5 border-b border-gray-50 bg-teal-50/30">
                ...
                <form action="{{ route('warga.payments.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4" x-data="{ nominal: '' }">
                    @csrf
                    <div>
                        <label class="text-[10px] font-black text-teal-600 uppercase px-1">Pilih Iuran</label>
                        <select name="contribution_id" class="w-full p-3 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-teal-500 text-sm font-bold"
                    ...
                    <div>
                        <label class="text-[10px] font-black text-teal-600 uppercase px-1">Nominal Otomatis</label>
                        <div class="relative">
                    ...
                    <div>
                        <label class="text-[10px] font-black text-teal-600 uppercase px-1">Tanggal Bayar</label>
                        <input type="date" name="tanggal_bayar" value="{{ date('Y-m-d') }}" class="w-full p-3 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-teal-500 text-sm font-bold" required>
                    </div>

                    <div>
                        <label class="text-[10px] font-black text-teal-600 uppercase px-1">Upload Bukti Transfer</label>
                        <input type="file" name="bukti_bayar" class="w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[10px] file:font-black file:bg-emerald-600 file:text-white hover:file:bg-emerald-800" required>
                    </div>

                    <button type="submit" class="w-full bg-emerald-600 text-white p-4 rounded-2xl font-black shadow-lg shadow-teal-100 hover:bg-emerald-900 transition-all uppercase text-xs tracking-widest">
                        Kirim ke Bendahara
                    </button>
                </form>
            </div>
                            ...
                            <tr class="hover:bg-teal-50/20 transition-colors">
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-[11px] text-gray-500 font-bold">
                                        {{ $p->tanggal_bayar ? \Carbon\Carbon::parse($p->tanggal_bayar)->format('d M Y') : \Carbon\Carbon::parse($p->created_at)->format('M Y') }} • 
                                        <span class="text-teal-600">Rp {{ number_format($p->jumlah_bayar, 0, ',', '.') }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    ...
                                    @elseif($p->status == 'unpaid')
                                        <span class="px-3 py-1 text-[9px] font-black bg-red-100 text-red-700 rounded-full uppercase italic animate-pulse">Belum Dibayar</span>
                                    @else
                                        <span class="px-3 py-1 text-[9px] font-black bg-emerald-100 text-emerald-700 rounded-full uppercase italic">Dicek</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-right">
                                    @if($p->status == 'unpaid')
                                        <button @click="$el.closest('body').querySelector('select[name=contribution_id]').value = '{{ $p->contribution_id }}'; $el.closest('body').querySelector('select[name=contribution_id]').dispatchEvent(new Event('change')); window.scrollTo({top:0, behavior:'smooth'});" 
                                                class="text-emerald-600 hover:text-white hover:bg-emerald-600 text-[10px] font-black uppercase tracking-widest bg-emerald-50 px-3 py-2 rounded-xl transition-all shadow-sm">
                                            Bayar Sekarang
                                        </button>
                                    @else
                                        <button @click="proofModal = true; activeImg = '{{ Storage::url($p->bukti_bayar) }}'" 
                                                class="text-teal-600 hover:text-teal-900 text-[10px] font-black uppercase tracking-widest bg-teal-50 px-3 py-2 rounded-xl transition-all">
                                            Lihat Bukti
                                        </button>
                                    @endif
                                </td>
